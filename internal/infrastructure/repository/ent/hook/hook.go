...
import (
	...
	"github.com/amr-mahmoud/mrktly/internal/infrastructure/repository/ent"
)
...
// The ProfessionalFunc type is an adapter to allow the use of ordinary
// function as Professional mutator.
type ProfessionalFunc func(context.Context, *ent.ProfessionalMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ProfessionalFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ProfessionalMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProfessionalMutation", m)
}
