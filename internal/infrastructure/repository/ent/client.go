...
import (
	...
	"github.com/amr-mahmoud/mrktly/internal/infrastructure/repository/ent/migrate"
...
type Client struct {
	...
	// Professional is the client for interacting with the Professional builders.
	Professional *ProfessionalClient
...
func (c *Client) init() {
	...
	c.Professional = NewProfessionalClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		Professional: NewProfessionalClient(cfg),
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.Professional.Intercept(interceptors...)
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *ProfessionalMutation:
		return c.Professional.mutate(ctx, m)
	case *TopicMutation:
...
// ProfessionalClient is a client for the Professional schema.
type ProfessionalClient struct {
	config
}

// NewProfessionalClient returns a client for the Professional from the given config.
func NewProfessionalClient(c config) *ProfessionalClient {
	return &ProfessionalClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `professional.Hooks(f(g(h())))`.
func (c *ProfessionalClient) Use(hooks ...Hook) {
	c.hooks.Professional = append(c.hooks.Professional, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `professional.Intercept(f(g(h())))`.
func (c *ProfessionalClient) Intercept(interceptors ...Interceptor) {
	c.inters.Professional = append(c.inters.Professional, interceptors...)
}

// Create returns a builder for creating a Professional entity.
func (c *ProfessionalClient) Create() *ProfessionalCreate {
	mutation := newProfessionalMutation(c.config, OpCreate)
	return &ProfessionalCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of Professional entities.
func (c *ProfessionalClient) CreateBulk(builders ...*ProfessionalCreate) *ProfessionalCreateBulk {
	return &ProfessionalCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *ProfessionalClient) MapCreateBulk(slice any, setFunc func(*ProfessionalCreate, int)) *ProfessionalCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &ProfessionalCreateBulk{err: fmt.Errorf("calling to ProfessionalClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*ProfessionalCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for Professional.
func (c *ProfessionalClient) Update() *ProfessionalUpdate {
	mutation := newProfessionalMutation(c.config, OpUpdate)
	return &ProfessionalUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *ProfessionalClient) UpdateOne(pr *Professional) *ProfessionalUpdateOne {
	mutation := newProfessionalMutation(c.config, OpUpdateOne, withProfessional(pr))
	return &ProfessionalUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *ProfessionalClient) UpdateOneID(id int) *ProfessionalUpdateOne {
	mutation := newProfessionalMutation(c.config, OpUpdateOne, withProfessionalID(id))
	return &ProfessionalUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Professional.
func (c *ProfessionalClient) Delete() *ProfessionalDelete {
	mutation := newProfessionalMutation(c.config, OpDelete)
	return &ProfessionalDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *ProfessionalClient) DeleteOne(pr *Professional) *ProfessionalDeleteOne {
	return c.DeleteOneID(pr.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ProfessionalClient) DeleteOneID(id int) *ProfessionalDeleteOne {
	builder := c.Delete().Where(professional.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &ProfessionalDeleteOne{builder}
}

// Query returns a query builder for Professional.
func (c *ProfessionalClient) Query() *ProfessionalQuery {
	return &ProfessionalQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeProfessional},
		inters: c.Interceptors(),
	}
}

// Get returns a Professional entity by its id.
func (c *ProfessionalClient) Get(ctx context.Context, id int) (*Professional, error) {
	return c.Query().Where(professional.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ProfessionalClient) GetX(ctx context.Context, id int) *Professional {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *ProfessionalClient) Hooks() []Hook {
	return c.hooks.Professional
}

// Interceptors returns the client interceptors.
func (c *ProfessionalClient) Interceptors() []Interceptor {
	return c.inters.Professional
}

func (c *ProfessionalClient) mutate(ctx context.Context, m *ProfessionalMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&ProfessionalCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&ProfessionalUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&ProfessionalUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&ProfessionalDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Professional mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		Customer, Professional, Topic, User []ent.Hook
	}
	inters struct {
		Customer, Professional, Topic, User []ent.Interceptor
	}
)
